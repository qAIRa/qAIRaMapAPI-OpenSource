...
def getAirDailyMeasurementsTimePeriod():
    ...
    init_week = int(request.args.get('init_week'))\
                if request.args.get('init_week') is not None else 30 
    init_year = int(request.args.get('init_year'))\
                if request.args.get('init_year') is not None else 2020 
    end_week = int(request.args.get('end_week'))\
                if request.args.get('end_week') is not None else 35 
    end_year = int(request.args.get('end_year'))\
                if request.args.get('end_year') is not None else 2020 
    try:
